...
class CityFlow_1x1_LowTraffic(gym.Env):
    ...
    def __init__(self):
        ...
        with open("/content/drive/MyDrive/gym_cityflow-master/gym_cityflow/envs/1x1_config/roadnet.json", "r") as read_content:
        ...
        for i in range(len(self.roadnet['intersections'])):
          if self.roadnet['intersections'][i]['id'] == self.intersection_id:
            idty = i
            self.lightphases = self.roadnet['intersections'][i]['trafficLight']['lightphases']
            self.road_phases = self.roadnet['intersections'][idty]['roadLinks']
        #print('HERE: ',self.lightphases)
        #print('HERE: ',self.road_phases[idty]['laneLinks'][0]['startLaneIndex'])
    ...
    def _get_reward(self):
        lane_waiting_vehicles_dict = self.cityflow.get_lane_waiting_vehicle_count()
        #print(lane_waiting_vehicles_dict)
        reward = 0.0
        reward_time = 0.0
        total_reward = 0.0
        lamb = 0.1
        #max_lane = max(lane_waiting_vehicles_dict, key=lane_waiting_vehicles_dict.get)
        ...
        if self.mode == "start_waiting":
            for (road_id, num_vehicles) in lane_waiting_vehicles_dict.items():
                if road_id in self.start_lane_ids:
                    reward_time -= self.sec_per_step * num_vehicles
        for i in range(len(self.road_phases)):
          start = self.road_phases[i]['startRoad']
          end   = self.road_phases[i]['endRoad']
          for j in range(len(self.road_phases[i]['laneLinks'])):
            start_lane = self.road_phases[i]['laneLinks'][j]['startLaneIndex']
            end_lane = self.road_phases[i]['laneLinks'][j]['endLaneIndex']
            start_lane_id = start + '_' + str(start_lane)
            end_lane_id = end + '_' + str(end_lane)
            pressure = lane_waiting_vehicles_dict[start_lane_id] - lane_waiting_vehicles_dict[end_lane_id]
            if pressure>-reward:
              reward = -pressure
        reward = reward + lamb*reward_time
        return reward
